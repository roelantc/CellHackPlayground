...
export class Cell {
    ...
    constructor(vector: Vector, energy: number, type : CellType = CellType.Rock, color : string = '#ff1100')
    {
        this.vector = vector;
    ...
    private moveCell(x: number, y: number)
    {
    ...
    private splitCell(x: number, y: number) {
    ...
    private isCellPositionFree(x: number, y: number)
    {
        return !cells.some(cell => cell.vector.x == x && cell.vector.y == y)
    }

    private eatCell(x: number, y: number) {
    ...
    private feedCell(x: number, y: number) {
        let neighbourCell = this.getCell(x, y);
        if (neighbourCell === undefined){
            return;
        }
    ...
    private decideCellAction(environment: Cell[]) {
